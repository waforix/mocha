import type { ChannelType, DataMode, InviteTargetType } from '../../enums';
import type {
  Application,
  AuditLogEntry,
  Channel,
 ...
 * GUILDS
 */
export type GuildCreate = EventData<
  Guild & {
    joinedAt: Date;
    large: boolean;
    unavailable: boolean;
...
export type ChannelPinsUpdate = EventData<
...
export type ThreadDelete = EventData<{
  id: string;
  guildId: string;
  parentId: string;
  type: ChannelType;
}>;
export type ThreadListSync = EventData<{
  guildId: string;
  channelIds?: string[];
  threads: Channel[];
...
export type ThreadMembersUpdate = EventData<{
  id: string;
  guildId: string;
  memberCount: number;
...
export type MessageReactionRemove = EventData<
  Omit<MessageReaction, 'member' | 'message_author_id' | 'burst_colors'>
>;
export type MessageReactionRemoveAll = EventData<{
  channelId: string;
  messageId: string;
  guildId?: string;
}>;
export type MessageReactionRemoveEmoji = EventData<{
  channelId: string;
  guildId?: string;
  messageId: string;
  emoji: Partial<Emoji>;
...
export type GuildScheduledEventUserAdd = EventData<{
  guildScheduledEventId: string;
  userId: string;
  guildId: string;
}>;
export type GuildScheduledEventUserRemove = EventData<{
  guildScheduledEventId: string;
  userId: string;
  guildId: string;
}>;
...
export type GuildMembersChunk = EventData<{
  guildId: string;
  members: GuildMember[];
  chunkIndex: number;
  chunkCount: number;
