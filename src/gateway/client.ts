...
export class GatewayClient extends EventEmitter {
    ...
    public connect(): void {
        ...
        this.webSocket.on(WebSocketEvent.CLOSE, (code: number, reason: Buffer) => this.onClose(code, reason));
    ...
    private onClose(code: CloseCode, reason: Buffer): void {
        ...
        if (code in FATAL_CLOSE_CODES) {
            ...
            this.emit("error", new Error(`Fatal gateway error (${code}): ${message} - ${reason.toString()}`));
            return;
        }
    ...
    private identify(): void {
        ...
        const payload = this.sessionId && this.resumeUrl
            ? {
                op: OpCode.RESUME,
                d: {
                    token: this.token,
    ...
    private validateToken(): void {
        if (!((/^[A-Za-z0-9_-]{24,}\.[A-Za-z0-9_-]{6}\.[A-Za-z0-9_-]{27,}$/)
            .test(this.token))
        ) {
            this.emit("error", new Error("Invalid token."));
            throw new Error("Invalid token format. Please enter a token in the following format: " +
