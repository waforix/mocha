...
export class Connection extends EventEmitter {
  ...
  public getHealth() {
    ...
    if (stats.reconnects > MAX_RECONNECT_RATE) {
      warnings.push(
        `High reconnection rate: ${stats.reconnects} reconnects. ` +
          'Check network stability and Discord API status.'
      );
    }
    if (stats.errors > MAX_ERROR_RATE) {
      warnings.push(
        `High error rate: ${stats.errors} errors. Review error logs and check token validity.`
      );
    }
    if (stats.averageLatency > MAX_AVERAGE_LATENCY) {
      warnings.push(
        `High latency: ${stats.averageLatency.toFixed(0)}ms. ` +
          'Check network connection and server location.'
      );
    }
    if (uptimePercentage < MIN_UPTIME_PERCENTAGE) {
      warnings.push(
        `Low uptime: ${uptimePercentage.toFixed(1)}%. Investigate connection stability issues.`
      );
    }
    return {
