...
import { RequestLimit } from "./types/requestLimit";
...
export class RateLimiter {
    ...
    public isRequestLimited(requestUrl: string): boolean {
        return false;
    }
    ...
    public async limit(): Promise<void> {
        while(true) {
            ...
            requestLimit.data.filter((i) =>
                    ((Date.now()- i) > requestLimit.interval))
                .shift();
            });
        }
    }
